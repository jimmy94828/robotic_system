...
def RRT_Core(start_uv, target_uv, free_safe, meta, step_size=0.5, max_iter=3000):            # change stepsize from 20 to 0.5
...
def rrt_planning(start, target, map_path, resolution, origin):
    ...
    # Bounds Check
    if not (0 <= start_uv[0] < W and 0 <= start_uv[1] < H): return []
    if not (0 <= target_uv[0] < W and 0 <= target_uv[1] < H): return []
