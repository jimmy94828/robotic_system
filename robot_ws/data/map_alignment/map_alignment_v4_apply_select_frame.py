...
def load_selected_frame_map(path: str) -> pd.DataFrame:
    """
    selected_frame_formatted.txt format:
      idx(from 0) origin-idx timestamp

    pose3d.idx matches idx(from 0), and pose2d.idx matches origin-idx.
    """
    rows = []
    for line_no, line in enumerate(Path(path).read_text().splitlines(), start=1):
        ...
        if len(toks) < 3:
            raise ValueError(
                f"selected frame line {line_no} needs 3 fields: pose3d_idx origin_idx timestamp, got: {line}"
            )
        rows.append({
            "pose3d_idx": int(toks[0]),
            "origin_idx": int(toks[1]),
            "timestamp": toks[2],
        })

    if not rows:
        raise ValueError(f"selected frame map is empty: {path}")

    df = pd.DataFrame(rows)
    dup_pose3d = df[df["pose3d_idx"].duplicated()]["pose3d_idx"].tolist()
    dup_origin = df[df["origin_idx"].duplicated()]["origin_idx"].tolist()
    if dup_pose3d:
        raise ValueError(f"selected frame map has duplicate pose3d_idx values: {dup_pose3d[:10]}")
    if dup_origin:
        raise ValueError(f"selected frame map has duplicate origin_idx values: {dup_origin[:10]}")
...
def main():
    ...
    ap.add_argument("--selected_frames", default="./input/selected_frame_formatted.txt",
                    help="frame map: pose3d_idx origin_pose2d_idx timestamp")
    ...
    ap.add_argument("--index_offset", type=int, default=0, help="deprecated: selected_frames supplies explicit mapping")
    ap.add_argument("--auto_offset", action="store_true", help="deprecated: ignored when selected_frames is used")
    ap.add_argument("--offset_range", type=int, default=5, help="deprecated: ignored when selected_frames is used")
    ...
    selected = load_selected_frame_map(args.selected_frames)
    if args.yaw_deg:
    ...
    def solve_for_selected_frames(T_cam_base: np.ndarray, tag: str):
        merged = (
            selected
            .merge(cam, left_on="pose3d_idx", right_on="idx", how="inner")
            .merge(base2d, left_on="origin_idx", right_on="idx", how="inner", suffixes=("_cam", "_base"))
        )
    ...
    for tag, T_cam_base in candidates:
        res = solve_for_selected_frames(T_cam_base, tag)
        if res is None:
            continue
    ...
    if best is None:
        raise RuntimeError("No valid solution: check selected_frames/min_matched/extrinsic.")

    print(
        f"[best] extrinsic={best['tag']}, selected_frames={args.selected_frames}, "
        f"matched={best['n_all']}, inliers={best['n_in']}, rmse={best['err']:.4f}"
    )
    ...
    for _, r in cam_tmp.iterrows():
        T_m3_c = cam_row_to_T(r)
        T_m3_b = T_m3_c @ T_cam_base
        cam_uv_all.append(project_to_plane_uv(T_m3_b[:3, 3], mu_plane, e1, e2))
    ...
    for _, r in merged.iterrows():
        T_m3_c = cam_row_to_T(r)
        T_m3_b = T_m3_c @ T_cam_base
        P3_uv_mat.append(project_to_plane_uv(T_m3_b[:3, 3], mu_plane, e1, e2))
    ...
    plt.ylabel("y")
    plt.title(
        f"Alignment: extrinsic={best['tag']}, selected mapping, RMSE={best['err']:.4f}, s={best['s']:.6f}, "
    ...
    out = {
        ...
        "selected_frames": str(args.selected_frames),
        "index_mapping": "selected.pose3d_idx == pose3d.idx; selected.origin_idx == pose2d.idx",
        ...
        "sim2": {
            "s": float(best["s"]),
            "R": [
                [float(best["R"][0, 0]), float(best["R"][0, 1])],
